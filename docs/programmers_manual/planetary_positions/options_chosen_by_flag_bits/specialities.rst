...
First call ``swe.set_topo()``. Then add the flag ``swe.FLG_TOPOCTR`` for the
call to ``swe.calc()``.
...
To compute a heliocentric position, add ``swe.FLG_HELCTR``.
...
``swe.FLG_BARYCTR`` yields coordinates as referred to the solar system
...
``swe.FLG_NOABERR`` and ``swe.FLG_NOGDEFL``. For positions related to the mean
equinox of 2000, you must set ``swe.FLG_J2000`` and ``swe.FLG_NONUT``, as well.

True or mean equinox of date
============================

``swe.calc()`` usually computes the positions as referred to the true equinox
of the date (i.e. with nutation). If you want the mean equinox, you can turn
nutation off, using the flag bit ``swe.FLG_NONUT``.

J2000 positions and positions referred to other equinoxes
=========================================================

``swe.calc()`` usually computes the positions as referred to the equinox of
date. ``swe.FLG_J2000`` yields data referred to the equinox J2000. For
positions referred to other equinoxes, ``swe.FLG_SIDEREAL`` has to be set and
the equinox specified by ``swe.set_sid_mode()``. For more information, refer to
chapter TODO

Sidereal positions
==================

To compute sidereal positions, set bit ``swe.FLG_SIDEREAL`` and use the
function ``swe.set_sid_mode()`` in order to define the ayanamsha you want.
...
precision using the flag ``swe.FLG_JPLHOR``. For best accuracy, the daily Earth
...
defined by ``swe.set_ephe_path()``. The EOP files are found on the IERS
...
``swe.FLG_JPLHORA``, which is a very good approximation of Horizons, at least
for 1962 to present.

``swe.FLG_JPLHORA`` can be used independently for the whole time range of the
Swiss Ephemeris.
